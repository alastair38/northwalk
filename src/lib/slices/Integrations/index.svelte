		...
		<div class="mt-20 flex flex-col items-center md:flex-row">
			{#each slice.primary.icons as item, index}
				{#if item.icon}
					...
					{/if}
					<div
						aria-label={`${item.icon} logo`}
						class="pulsing-icon flex aspect-square h-12 w-12 shrink-0 items-center justify-center rounded-full border border-violet-50/30 bg-violet-50/25 p-3 text-3xl text-violet-100 opacity-40 md:h-20 md:w-20 md:text-3xl lg:text-5xl"
					>
						<svelte:component this={icons[item.icon]} aria-hidden="true" />
					</div>
