...
>
	<div class="flex items-center gap-6">
		...
		<button
			class="focus:online-none absolute bottom-2 right-2 rounded-full bg-brand p-1 text-base ring-offset-2 hover:bg-content-light focus-visible:ring-2 focus-visible:ring-content"
		...
		<div class="mb-12 flex flex-col gap-6 p-6">
			...
			{#each searchData?.data?.publications as item}
				<div class=" border-b py-2">
					<PrismicLink
						document={item}
						class="group flex justify-between font-bold text-content transition-colors duration-200 focus-visible:rounded-md   focus-visible:text-content/50 focus-visible:outline focus-visible:outline-offset-4 focus-visible:outline-brand/50"
						on:click={closeDialog}
					>
						<span class="hover:text-content/50"> <PrismicText field={item.data.company} /></span>
						<span class="rounded-full bg-brand-light/30 px-3 py-1 text-sm font-light capitalize">
